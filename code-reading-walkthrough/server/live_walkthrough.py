#!/usr/bin/env python3
"""
live_walkthrough.py — open a rendered walkthrough HTML with live-mode Q&A enabled.
...
Usage:
    python3 live_walkthrough.py <path/to/walkthrough.html>     # start (or reuse) and open browser
    python3 live_walkthrough.py <html> --no-open               # just start, don't open
    python3 live_walkthrough.py --status                       # list running live servers
    python3 live_walkthrough.py --stop                         # stop all live-walkthrough servers
    python3 live_walkthrough.py --stop <html>                  # stop the one serving <html>
...
files live under /tmp/live-walkthrough-<port>.{pid,log}.
"""
...
def pid_file(port): return PID_DIR / f"live-walkthrough-{port}.pid"
def log_file(port): return PID_DIR / f"live-walkthrough-{port}.log"
...
def cmd_start(args):
    ...
    if existing_port is not None:
        ...
        print(f"[live-walkthrough] reusing existing server on port {existing_port} (cli={existing_info.get('cli')})")
        print(f"[live-walkthrough] open: {url}")
        if not args.no_open:
            webbrowser.open(url)
        return
    ...
    if port != args.port:
        print(f"[live-walkthrough] port {args.port} busy with another file; using {port} instead")
    ...
    print(f"[live-walkthrough] started server pid={pid} on port {port} (cli={info.get('cli')})")
    print(f"[live-walkthrough] log: {log_file(port)}")
    print(f"[live-walkthrough] open: {url}")
    if not args.no_open:
        webbrowser.open(url)
...
def cmd_status(args):
    ...
    if not found:
        print("(no live-walkthrough servers running on ports {}–{})".format(min(PORT_RANGE), max(PORT_RANGE)))
        return
    ...
    for port in PORT_RANGE:
        ...
        if pid is not None:
            try:
                ...
                pf.unlink()
                stopped += 1
                print(f"[live-walkthrough] stopped pid={pid} on port {port}")
                continue
            except ProcessLookupError:
                pf.unlink()
            except OSError as e:
                print(f"[live-walkthrough] could not signal pid={pid}: {e}", file=sys.stderr)
        # Fallback: no pid file (rare) — we know it's alive but can't target it
        print(f"[live-walkthrough] port {port} has a server but no pid file at {pf}; skipping")
    if stopped == 0:
        if target:
            print(f"(no live-walkthrough server found serving {target})")
        else:
            print("(no live-walkthrough servers to stop)")


def parse_args():
    p = argparse.ArgumentParser(
        description="Start (or reuse) the code-reading-walkthrough live_server and open the rendered HTML in a browser.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument("html", type=Path, nargs="?", help="Path to the rendered walkthrough HTML.")
    ...
    p.add_argument("--status", action="store_true", help="List running live-walkthrough servers and exit.")
    p.add_argument("--stop", action="store_true",
                   help="Stop the server serving <html>, or all live-walkthrough servers if no <html> given.")
    return p.parse_args()
...
def main():
    ...
    if args.status:
        ...
        if not args.html:
            print("usage: live_walkthrough.py <html> | --status | --stop [<html>]", file=sys.stderr)
            sys.exit(2)
        cmd_start(args)
